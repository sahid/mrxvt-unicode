...
rxvt_scr_alloc (rxvt_t* r, int page)
{
    ...
    */
    ...
    PVTS(r, page)->wdrawn_text = rxvt_calloc(nrow, sizeof(wtext_t*));
    PVTS(r, page)->drawn_rend = rxvt_calloc(nrow, sizeof(rend_t*));

    PSCR(r, page).text = rxvt_calloc(total_rows, sizeof(text_t*));
    PSCR(r, page).wtext = rxvt_calloc(total_rows, sizeof(wtext_t*));
...
rxvt_scr_reset_realloc(rxvt_t* r, int page)
{
    ...
    PSCR(r, page).wtext = rxvt_realloc (
	PSCR(r, page).wtext, total_rows * sizeof(wtext_t *));
    ...
    PVTS(r, page)->wdrawn_text  = rxvt_realloc (
	PVTS(r, page)->wdrawn_text, nrow * sizeof(wtext_t *));
...
rxvt_scr_adjust_col (rxvt_t* r, int page, unsigned int total_rows)
{
	...
	PVTS(r, page)->wdrawn_text[p] = rxvt_realloc (
	    PVTS(r, page)->drawn_text[p], ncol * sizeof(wtext_t));
...
/*
 * Add text given in <str> of <len> characters to screen struct
 */
/* EXTPROTO */
void
mrxvt_scr_add_lines(rxvt_t* r, int page, const wchar_t *str, int nlines,
	int len)
{
	unsigned char   checksel, clearsel;
	wchar_t	    c;
	...
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "mrxvt_scr_add_lines (r, %d, %.*s, %d, %d)\n", page, min(len, 36), str, nlines, len));
	...
	for (i = 0; i < len;)
	{
		...
		switch (c)
		{
			...
			case '\n':
				...
				stp = PSCR(r, page).wtext[row];  /* _must_ refresh */
				srp = PSCR(r, page).rend[row];  /* _must_ refresh */
				RESET_CHSTAT(r, page);
				continue;
	...
	//void (*xftdraw_string)())
{
	...
	//rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "Draw: 0x%8x %p: '%.40s' (%i)\n", rend, font, str, len ));
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "Draw: 0x%8x %p: '%i' (%i)\n", rend, font, *str, len ));
...
# endif	/* TEXT_SHADOW */
	...
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "\e[32mNow I draw %i\e[0m\n", *str));
	//XFTDRAW_STRING (win, fore, font, x, y, "\e[32mtest\e[0m\n", strlen("\e[32mtest\e[0m\n"));

	XftDrawString32 (win, fore, font, x, y, (FcChar32*) str, len);
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "\e[32mNow I have drawn %i\e[0m\n", *str));
	//XwcDrawString (win, fore, xftfont, x, y, fc, lenglyphs); -> for non xft!
...
/* INTPROTO */
void // TODO
wrxvt_scr_draw_string (rxvt_t* r, int page,
	int x, int y, wchar_t* str, int len, int drawfunc,
	uint16_t fore, uint16_t back,
	__attribute__((unused)) rend_t rend, Region refreshRegion)
{
#ifdef XFT_SUPPORT
	...
	switch (drawfunc)
	{
		...
		case	XFT_DRAW_IMAGE_STRING_16:
			fillback = 1;
		case	XFT_DRAW_STRING_16:
			//xftdraw_string = XftDrawString16;
			break;
	}
...
# endif	/* MULTICHAR_SET */
			...
			/*wrxvt_draw_string_xft(r, PVTS( r, page)->vt, r->TermWin.gc,
					refreshRegion, rend, NO_PFONT,
					PVTS(r, page)->xftvt, &(r->xftColors[fore]),
					x, y, pstr, plen);*/
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "\e[32mhello1\e[0m\n"));
			wrxvt_draw_string_xft(r, PVTS( r, page)->vt, r->TermWin.gc,
					refreshRegion, rend, NO_PFONT,
					PVTS(r, page)->xftvt, &(r->xftColors[fore]),
					x, y, str, len);

	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "\e[32mhello2\e[0m\n"));
			x += Width2Pixel (loopitem);
			newstr += loopitem;	/* next string to display */
		}
	}
...
#endif	/* XFT_SUPPORT */
	{
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "\e[32mhello3\e[0m\n"));
		int	(*draw_string) ();
		switch (drawfunc)
		{
			case    X11_DRAW_STRING_8:
...
void
mrxvt_scr_refresh(rxvt_t* r, int page, unsigned char refresh_type)
{
	rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "mrxvt_scr_refresh (r, %i).\n", page));
	unsigned char
		clearfirst, /* first character writes before cell */
		clearlast,  /* last character writes beyond cell */
		must_clear, /* use drawfunc not image_drawfunc */
...
# endif
	...
	rend_t*	drp;	    /* drawn_rend pointer */
	rend_t*	srp;	    /* screen-rend-pointer */
	wtext_t*	dtp;	    /* drawn-text pointer */
	wtext_t*	stp;	    /* screen-text-pointer */
	wchar_t*	buffer;	    /* local copy of r->h->buffer */
	/*
		int		(*drawfunc) () = XDrawString;
		int		(*image_drawfunc) () = XDrawImageString;
		*/
...
# endif
	...
	{
		h->currmaxcol = r->TermWin.ncol;
		h->wbuffer = rxvt_realloc(h->wbuffer, sizeof(char) * (h->currmaxcol + 1));
	}
	buffer = h->wbuffer;
...
# endif
			...
			!must_clear)
	{
		...
		for (; j-- >= 0; row += (i > 0 ? 1 : -1))
		{
			if (row + i >= 0 && row + i < r->TermWin.nrow && row + i != ocrow)
			{
				stp = PSCR(r, page).wtext[row + row_offset];
				srp = PSCR(r, page).rend[row + row_offset];
				dtp = PVTS(r, page)->wdrawn_text[row];
				dtp2 = PVTS(r, page)->wdrawn_text[row + i];
				drp = PVTS(r, page)->drawn_rend[row];
				drp2 = PVTS(r, page)->drawn_rend[row + i];
				for (nits = 0, col = r->TermWin.ncol; col--; )
					if (stp[col] != dtp2[col] ||
	...
	for (row = 0; row < r->TermWin.nrow; row++)
	{
		...
		for (col = 0; col < r->TermWin.ncol; col++)
		{
			...
			t = dtp[col]; // TODO (Jehan): initialize this!
			is_same_char = (t == stp[col] && drp[col] == srp[col]);
			if (!clear_next &&
					(is_same_char || t == 0 || t == ' '))
				/* screen cleared elsewhere */
...
# endif
		...
		for (col = 0; col < r->TermWin.ncol; col++)
		{
			...
			/* redraw one or more characters */
			rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "for loop (%i)\n", col));
...
# endif
			...
			if (back == Color_bg && must_clear)
			{
				...
				for (i = 0; i < len; i++)
					...
					if (buffer[i] != ' ')
					{
						...
						// TODO: test avec wchar_t
						//wchar_t* text = malloc (sizeof (wchar_t) * 100);
						//int countwc = mbsrtowcs (text, (const char**) &buffer, 100, NULL);
						wrxvt_scr_draw_string (r, page, xpixel, ypixelc,
								buffer, wlen, drawfunc,
								//text, countwc, drawfunc,
								fore, back, rend,
								((refresh_type & CLIPPED_REFRESH) ?
								 r->h->refreshRegion : None ));
						break;
			...
			{
				// TODO: test avec wchar_t
				rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "NC Drawing '%s' (%d)\n", buffer, len));
				//wchar_t* text = malloc (sizeof (wchar_t) * 100);
				//int countwc = mbsrtowcs (text, (const char**) &buffer, 100, NULL);
				wrxvt_scr_draw_string (r, page,
						xpixel, ypixelc,
						buffer, wlen, image_drawfunc,
						//text, countwc, image_drawfunc,
						fore, back, rend,
						((refresh_type & CLIPPED_REFRESH) ?
						 r->h->refreshRegion : None ));
				rxvt_dbgmsg ((DBG_DEBUG, DBG_SCREEN, "NC Drawing '%.60s' (%d)\n", buffer, len));

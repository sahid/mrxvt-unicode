...
{
    text_t* in_str_copy = in_str;
#ifdef HAVE_ICONV_H
    char** in_text = (char**) &in_str_copy;
#else
    wchar_t** in_bytes = (wchar_t) &in_str_copy;
#endif
    size_t in_str_left = in_str_size;
    // The returned text will be at most the number of bytes.
    size_t out_str_left = in_str_size * sizeof (text_t);
...
/* Stolen verbatim (and indented to ts=4) from glibc-2.3.5. Replacement for
 * cygwin's missing strcasestr function. */
